...
exports.formatMarkdown = void 0;
function formatMarkdown(allProblems, byType, bySource, byCode) {
...
function formatProblem(problem, report) {
    ...
    if (problem.relatedInfo.length > 0) {
        ...
        problem.relatedInfo.forEach(info => {
            ...
            report += `    at \
      \
      \`${info.filename}\` Line ${info.location.startLine}, Column ${info.location.startColumn}\n`;
        });
    }
    report += '\n';
}
